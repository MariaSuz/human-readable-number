module.exports = function toReadable (number)  {
    ...
    if (20 <= number) {
        ...
        const num = String(number).split('');  //Разбивает еа 2 строки, например 21 = ['2', '1']
        if (num.length === 2)
            if (number % 10 == 0) {    //Если ровно 20, то вернём из списка dozens
                return dozens[number];
            } else {
                if (num[0] == 2) {
                    return 'twenty' + ' ' + words[num[1]]; // words[num[1]] - это 1 если число было 21, те  ['2', '1']
                }
                if (num[0] == 3) {
